...
interface Theme {
  bg: string;
  lineActive: ThemeColor;
  nodeActive: ThemeColor;
  glow: string;
  ripple: string;
}

type GlobalColor = "default" | "monochrome";

(function () {
  let GLOBAL_COLOR: GlobalColor = "default";
  ...
  const LINE_BASE: ThemeColor = { r: 255, g: 255, b: 255, a: 0.13 };
  const NODE_BASE_RADIUS = 1.8;
  const NODE_ACTIVE_RADIUS = 3.2;

  const THEMES: Record<GlobalColor, Theme> = {
    default: {
      bg: "#161618",
  ...
  function lerpColor(base: ThemeColor, active: ThemeColor, t: number): string {
  ...
  const root = document.getElementById("kinetic-grid") as HTMLDivElement;
  const canvas = document.getElementById("kinetic-canvas") as HTMLCanvasElement;
  const ctx = canvas.getContext("2d") as CanvasRenderingContext2D;

  if ((GLOBAL_COLOR as string) === "monochrome") {
  ...
  function draw(now: number): void {
    const W = size.w;
    const H = size.h;
    const theme = THEMES[GLOBAL_COLOR] || THEMES.default;
    ...
    function drawSeg(p1: Point, p2: Point, pr1: number, pr2: number): void {
      const avg = (pr1 + pr2) / 2;
