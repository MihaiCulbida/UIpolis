type FormShape = "Checks" | "Stripes" | "Edge";

const FormShapes: Record<FormShape, number> = {
  Checks: 0,
  Stripes: 1,
  Edge: 2,
};
...
const presets: Record<PresetName, FlowParams> = {
  Aurora: {
...
class FlowField {
  ...
  constructor(container: HTMLElement, canvas: HTMLCanvasElement, options: GradientOptions = {}) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const grain = document.getElementById("gradient-grain") as HTMLDivElement | null;

  if (container && canvas) {
    try {
      new FlowField(container, canvas, { preset: "Aurora" });
      if (grain) grain.style.opacity = "0.15";
    } catch (e) {
