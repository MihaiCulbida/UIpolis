...
(function () {
  ...
  function initGradient(host: HTMLDivElement, el: HTMLDivElement): void {
    ...
    function frame(): void {
      ...
      if (width <= CONFIG.startingGap - CONFIG.breathingRange) directionWidth = 1;
      if (!CONFIG.breathing || reduced) directionWidth = 0;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const host = document.getElementById("gradient-host") as HTMLDivElement | null;
    const el = document.getElementById("gradient-el") as HTMLDivElement | null;
    if (host && el) initGradient(host, el);
  });
})();
